.phenosigdb_package_version <- "0.1.20"
...
.phenosigdb_core_url <- function(reference_species = "human") {
  ...
  paste0("https://raw.githubusercontent.com/GeNeHetX/phenosigdb/", ref, "/data/", parquet_name)
}
...
.phenosigdb_install_direct_resource <- function(resource, verbose = TRUE) {
  ...
  metadata <- if (length(metadata_rows)) do.call(rbind, metadata_rows) else data.frame(stats::setNames(replicate(length(.phenosigdb_resource_metadata_columns), logical(0), simplify = FALSE), .phenosigdb_resource_metadata_columns))
...
.phenosigdb_optional_metadata <- function(reference_species = "human") {
  ...
  if (!length(frames)) {
    return(data.frame(stats::setNames(replicate(length(.phenosigdb_resource_metadata_columns), logical(0), simplify = FALSE), .phenosigdb_resource_metadata_columns)))
  }
