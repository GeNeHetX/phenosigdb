...
def validate_database(data: str | Path | pd.DataFrame | None = None, reference_species: str = "original") -> pd.DataFrame:
    ...
    if "weight" in frame.columns:
        ...
        mixed_format = frame.groupby("signature_id", sort=True)["weight"].apply(lambda values: values.notna().any() and values.isna().any())
        if mixed_format.any():
            bad = mixed_format[mixed_format].index.tolist()[:5]
            raise ValueError(f"Some signatures mix weighted and unweighted rows: {bad}")
