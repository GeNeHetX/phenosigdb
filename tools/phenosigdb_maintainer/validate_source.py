...
Usage as script:
    python tools/phenosigdb-validate-source [SOURCE_KEY | --changed | --all]
"""
...
def load_source(source_key: str, curated_dir: Path | None = None) -> pd.DataFrame:
    ...
    if curated_dir is None:
        ...
        curated_dir = repo_root / "signatures" / "curated" / source_key
    else:
...
def get_changed_sources() -> list[str]:
    ...
    for f in changed_files:
        if f.startswith("signatures/curated/"):
            parts = f.split("/")
            if len(parts) >= 3:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python tools/phenosigdb-validate-source CAF.Xing21
  python tools/phenosigdb-validate-source --changed
  python tools/phenosigdb-validate-source --all
        """
    )
    parser.add_argument(
        "source_key",
    ...
    if args.changed:
        ...
        repo_root = Path(__file__).resolve().parents[2]
        curated_dir = repo_root / "signatures" / "curated"
        sources = []
